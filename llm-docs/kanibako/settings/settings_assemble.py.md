...
⚑⚑ **The agent file's top-level table is `self:`, NOT `agent:`.** `agent_config.agent_file_route` is
the SoT for the per-agent file shape, and `_agent_partial` reads `raw["self"]`. *(The `_agent_partial`
...
* `(("self",), 0)` — agent file: `self.<leaf>` (`agent_config.agent_file_route`)
...
sub-table becomes THIS level; the two are kept SEPARATE cascade levels (spec §2), whereas
`config.read_agent_settings` pre-merges its own file's equivalent sections.

The per-agent DISCRIMINATOR is KEPT VERBATIM — the sub-table is re-rooted under its TRUE
discriminated name `agent.<sub_key>` (`agent.default.<key>` for the default layer,
`agent.<active-name>.<key>` for the active layer), the ONLY two agent key forms the spec defines
(§2d; §0 forbids a bare `agent.<key>`). The two agent levels then merge BY THEIR TRUE NAMES, each
scope-qualified key overriding the same key at a lower level; the active-over-default value-pick is a
thin effective-agent READ deferred to the consumer — the cascade's job is precedence by LEVEL ORDER,
not a name collapse. This preserves §0 per-agent independence: a box/workset that sets
