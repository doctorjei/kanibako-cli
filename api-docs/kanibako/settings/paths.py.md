...
_XDG_SPEC_DEFAULTS: dict[str, str] = {'XDG_DATA_HOME': '.local/share', 'XDG_CONFIG_HOME': '.config', 'XDG_STATE_HOME': '.local/state', 'XDG_CACHE_HOME': '.cache'}
...
def resolve_config_paths(set_values: Mapping[str, str], *, data_home: Path, home: Path) -> dict[str, str]:
    ...
...
def iter_workset_projects(std: StandardPaths, config: KanibakoConfig) -> list[tuple[str, _WorksetLike, list[tuple[str, str]]]]:
    ...
