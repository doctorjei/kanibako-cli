...
This document serves as a starting point; it can be extended or replaced.

> Seeded into the agent's store (`agents/<agent>/canon/handbook`, from the plugin's base or the agent-default template); delivered read-only at `~/canon/handbook/agent`.
> Directives that would apply to this agent (persona + harness), across every box it runs in, go here.
