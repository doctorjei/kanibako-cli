...
# caller of pid-add.sh: our own $PPID is the hook shell, not the agent.  The bare
# $PPID fallback is correct only when this script IS the hook command.
~/canon/charter/general/scripts/util/pid-add.sh "${1:-$PPID}"
...
handbook_hook=~/canon/handbook/general/scripts/hooks/clear-start.sh
