"""Bootstrap path literals — THE one home for hardcoded path values.

🚧 MIGRATION IN PROGRESS (step 1 of 4, 2026-09-02). This file is a VERBATIM COPY of
``paths_defaults.py``; the two deliberately hold the same constants until step 3
deletes the path values from ``paths_defaults`` and the non-path text from here.
🛑 The duplication is a MIGRATION, not a second carrier — do not "fix" it by hand.
⚑ Consumers of the PATH symbols already point here; nothing points here for the
``MSG_``/``WARN_``/``ERR_`` text, which stays behind.
⚑ ``RUN_USER_UID_PATH`` is DEFERRED: it is spliced into ``WARN_RUNDIR_UNUSABLE``, and
moving it would add an import to ``paths_defaults``, whose import-free-ness is a
CYCLE GUARD (``test_paths_defaults_is_import_free``).
"""
